...
# Setting seed so model is deterministic for each run with repeatable results.
random.seed(0)
...
class TranscriptionHandler:
    def __init__(self, model_name="base", download_root="./models"):
        ...
        print(
            f"Loading model '{model_name}' from {download_root}. This may take a few moments..."
        )
    ...
    def __call__(self, file_path):
        ...
        if processed_lyrics:
            result = self._align_and_transcribe(
                file_path, processed_lyrics, base_file_name
            )
        else:
